...
export function DnsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      setItems((await api.get<DnsUpdateStatus[]>("/api/dns/status")) ?? []);
    } finally {
      setLoading(false);
    }
  }
      ...
      <Card>
          ...
          action={
            <>
              ...
              <Button
                ...
                onClick={() =>
                  act(
                    () => api.post<string>("/api/dns/test-auth"),
                    "凭证测试请求已完成",
                  )
                }
              >
              ...
              <Button
                ...
                onClick={() =>
                  act(
                    () => api.post<string>("/api/dns/test-record", body),
                    "记录配置测试完成",
                  )
                }
              >
              ...
              <Button
                onClick={() =>
                  act(
                    () => api.post<DnsUpdateStatus[]>("/api/dns/update", body),
                    "DNS 更新已触发",
                  )
                }
              >
              ...
              <Button
                ...
                onClick={() =>
                  act(
                    () => api.post<string>("/api/dns/trigger-test", body),
                    "已触发测速",
                  )
                }
              >
        ...
        <CardBody>
          {loading ? (
            ...
            <Empty title="暂无 DNS 状态" />
          ) : (
            <div className="grid gap-4 lg:grid-cols-3">
              {items
                .filter((x) => !isp || x.isp === isp)
                .map((d) => (
                  <div
                    key={d.isp}
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <Badge tone={d.success ? "success" : "warning"}>
                        {d.success ? "成功" : "异常"}
                      </Badge>
                    </div>
                    ...
                    <div className="mt-1 text-xs text-fg-muted">
                      更新时间：{formatDateTime(d.lastUpdatedAt)}
                    </div>
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <span className="text-fg-muted">
                              ...
                              {formatLossRate(r.packetLossRate)}
                            </span>
                          </div>
                        </div>
                      ))}
