  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader
          title="IP 池"
          desc="按运营商维护手动 IP 与 API 拉取 IP"
          action={<>
            <Select value={isp} onChange={(e) => setIsp(e.target.value as IspKey)}>
              {ISP_KEYS.map((k) => <option key={k} value={k}>{ispLabel(k)}</option>)}
            </Select>
            <Button variant="secondary" onClick={load}><RefreshCw className="h-4 w-4" />刷新</Button>
            <Button onClick={refresh}>拉取 API 池</Button>
          </>}
        />
        <CardBody>
          <div className="grid gap-6 lg:grid-cols-[460px_1fr]">
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-[150px_1fr]">
                <Field label="预览源类型">
                  <Select value={previewType} onChange={(e) => setPreviewType(Number(e.target.value) as SourceType)}>
                    <option value={0}>API 源</option>
                    <option value={1}>CNAME 源</option>
                  </Select>
                </Field>
                <Field label="添加 IP / 预览源">
                  <Textarea
                    value={ips}
                    onChange={(e) => setIps(e.target.value)}
                    placeholder={previewType === 0 ? "填写 API URL；添加 IP 时可输入多个 IP" : "填写 CNAME 域名；添加 IP 时可输入多个 IP"}
                  />
                </Field>
              </div>

              {sources.length > 0 && <div className="rounded-xl border border-border bg-surface p-3">
                <div className="mb-2 text-xs font-medium text-fg-muted">配置里的拉取源</div>
                <div className="space-y-2">
                  {sources.map((s, i) => {
                    const sourceType = Number(s.type) as SourceType;
                    return (
                      <div key={`${s.type}-${s.value}-${i}`} className="flex items-center gap-2 rounded-lg bg-card px-3 py-2 text-sm hover:bg-card-hover">
                        <button
                          type="button"
                          onClick={() => { setPreviewType(sourceType); setIps(s.value); }}
                          className="flex min-w-0 flex-1 items-center gap-2 text-left"
                        >
                          <Badge tone={sourceType === 0 ? "info" : "warning"}>{sourceType === 0 ? "API" : "CNAME"}</Badge>
                          <span className="min-w-0 truncate font-mono text-xs">{s.value}</span>
                        </button>
                        <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={() => removeSource(s)} title="删除拉取源">
              ...
              {preview && <div>
                <div className="mb-2 text-sm font-medium">预览结果（{preview.length}）</div>
                <CodeBox value={preview.join("\n")} />
              </div>}
            </div>

            <div className="grid gap-4 xl:grid-cols-2">
              <IpList title="手动 IP" source="manual" ips={current.manualIps} onRemove={remove} />
              <IpList title="API IP" source="api" ips={current.apiIps} onRemove={remove} />
            </div>
          </div>
        </CardBody>
      </Card>
      ...
      <Modal
        ...
        footer={<><Button variant="secondary" onClick={() => setReplaceOpen(false)}>取消</Button><Button variant="danger" onClick={replace}>确认覆盖</Button></>}
      >
        <p className="text-sm text-fg-muted">这会用输入框中的 {inputIps.length} 个 IP 覆盖当前 {ispLabel(isp)} 的手动 IP 池。</p>
      </Modal>
    </div>
  );
}
